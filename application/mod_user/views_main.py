...
def main_top(page_number):
    return _load_page_with_entries(title=u'Top plusowane', order_by="votes_up desc", page_number=page_number)

def _load_page_with_entries(title=None, page_number=None, order_by=None):
    items_per_page = app.config['ITEMS_PER_PAGE']
    response, status = api_get_entries(order_by=order_by, per_page=items_per_page, page_number=page_number)
