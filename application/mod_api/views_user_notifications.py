...
def api_get_active_user_notifications(user_token=None):
    ...
    # Get notifications
    notifications = UserNotificationDAO.get_active_notifications(user_token)
    return flask.jsonify({'notifications': [n.to_json() for n in notifications]}), 200


@app.route('/api/user_notifications/active/count', methods=['GET'])
def api_get_active_user_notifications_count(user_token=None):
    # Get params
    if user_token is None:
        user_token = flask.request.args.get('user_token')
...
def api_dismiss_all_user_notifications(user_token=None):
    ...
    UserNotificationDAO.dismiss_all_notifications(user_token)
    return flask.jsonify({'dismissed': 'ok'}), 200
